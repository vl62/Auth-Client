...
class Varioml_model extends CI_Model {

	public function parseVarioML($xml) {
		$entryxml = @simplexml_load_string($xml);
//		print_r($entryxml);
		$source_id = $entryxml->source['id'];
		$source_name = $entryxml->source->name;
		$email = $entryxml->source->contact->email;
		$name = $entryxml->source->contact->name;
		echo "-> $source_id $source_name $name $email\n";
		$variant_count = count($entryxml->variant);
		echo "variant count -> $variant_count\n";
		foreach ($entryxml->variant as $variant ) {
//			print_r($variant);
			$ref = $variant->ref_seq['accession'];
			$hgvs = $variant->name;
			$gene = $variant->gene['accession'];
			$laboratory_id = $variant->source['id'];
			$phenotype = $variant->panel->phenotype['term'];
			$phenotype_omim = $variant->panel->phenotype['accession'];
			$phenotype_source = $variant->panel->phenotype['source'];
			$pathogenicity = $variant->pathogenicity['term'];
			$build = $variant->location->ref_seq['accession'];
			$location_ref = $variant->location->chr;
			$start = $variant->location->start; 
			$end = $variant->location->end;

//
			if ( isset($variant->panel->individual->gender['code']) ) {
				$gender_code = (string) $variant->panel->individual->gender['code'];
				$gender = $this->_getGenderFromGenderCode($gender_code);
			}
			print "gender -> $gender -> " . $variant->panel->individual->gender['code'] . "\n";
...
//		if ( $entryxml = @simplexml_load_string($xml) ) {
