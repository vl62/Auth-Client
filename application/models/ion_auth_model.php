...
//			$db_password =  $salt . substr(sha1($salt . $password), 0, -$this->salt_length);
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			// Owen - added to automatically log the user in when the activation is successful - do this by getting the user details and setting them in the session (as done in normal login function)
...
//			error_log(print_r($user, TRUE));
			...
			// Update the login statistics, only possible if curl is enabled and stats logging is enabled
			if ( $this->config->item('stats')) {
				if (in_array  ('curl', get_loaded_extensions())) {
					...
					updateStats($login_stats, 'loginstats');
				}
			}
...
//		error_log("activation -> " . $manual_activation);
...
//		$password   = $this->hash_password($password, $salt);
		...
		$data = array(
		    ...
		    'created_on' => time(),
		    'last_login' => time(),
		    'active'     => ($manual_activation === 1 ? 1 : 0)
		);
		...
		// Update the registration statistics, only possible if curl is enabled and stats reporting is enabled
		if ( $this->config->item('stats')) {
			if (in_array  ('curl', get_loaded_extensions())) {
				$login_stats = array(
					'username' => $username,
				...
				$this->load->helper('cafevariome' );
				updateStats($login_stats, 'registrationstats');
			}
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE) {
				...
				// Update the login statistics, only possible if curl is enabled and stats logging is enabled - TODO: Switch to non-curl
				if ( $this->config->item('stats')) {
					if (in_array  ('curl', get_loaded_extensions())) {
						$ip = getRealIpAddr();
						$login_stats = array(
	...
	// Owen - rewrote this as the function in_group from the Ion_auth libarary didn't work
	public function check_if_in_group($check_group, $id=false) {
		$users_groups = $this->get_users_groups($id)->result();
		$groups_array = array();
		foreach ($users_groups as $group) {
			$groups_array[] = $group->id;
//			error_log("id " . $group->id . " name " . $group->name);
		}
		if (in_array($check_group, $groups_array)) {
			return TRUE;
		}
//		else {
//			error_log("not in group!!!!");
//		}
		return FALSE;
	}
...
//		print $this->db->last_query() . "<br />";
	...
	// Owen - added to just get normal array for groups
...
//		print $this->db->last_query() . "<br />";
	...
	// Owen - added to just get array of arrays containing all group info
	public function getGroupsFull() {
		$this->db->select('id, name, description');
		$query = $this->db->get($this->tables['groups']);
		$groups = array();
		foreach ($query->result_array() as $row) {
			$groups[$row['id']] = array( "id" => $row['id'], "name" => $row['name'], "description" => $row['description']);
		}
//		print $this->db->last_query() . "<br />";
		return $groups;
	}
...
//		error_log($this->db->last_query());
	...
	**/
	...
	protected function _prepare_ip($ip_address) {
		...
		{
//			return inet_pton($ip_address); // Owen - removed as want IP as human readable format not packed in_addr
			return $ip_address;
		}
	}
}
