...
//					});
                ...
                if (!alert(numLeads + " emails successfully sent")) {
                    window.location.reload();
                }
            }
        });
...
//			alert("--> " + checkedLeadIDs);
            $.ajax({url: baseurl + 'admin/delete_crm_leads',
                ...
                dataType: 'html',
                success: function (data) {
                    if (!alert(numLeads + " lead(s) successfully deleted")) {
                        window.location.reload();
                    }
                }
            });
...
function show_growl_elastic_search() {
    $.growl.warning({message: "Changes made to variants and/or phenotypes. <a href='admin/redirect_to_elastic_search'>Click here</a> to re-build elastic search engine.",
        duration: 1E11});
}