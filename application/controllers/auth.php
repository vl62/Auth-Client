	...
	function login()
	{
		if ($this->ion_auth->logged_in()) {
			redirect('/', 'refresh');
                ...
                $fp = @fsockopen("www112.lamp.le.ac.uk", 80, $errno, $errstr, 30);
                if ($fp) {
                    redirect(base_url("/auth_federated/login"));
                }
...
//			$remember = (bool) $this->input->post('remember');
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), 0))
			{
...
//		redirect('auth/login', 'refresh');
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			//render
			$this->_render('auth/change_password');
//			$this->load->view('auth/change_password', $this->data);
	...
	function forgot_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->_render('auth/forgot_password');
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user)
		{
			...
			if ($this->form_validation->run() == false)
			{
				...
				//render
				$this->_render('auth/reset_password');
...
//				if ($this->_valid_csrf_nonce() === FALSE || $user->id != $this->input->post('user_id'))
	...
	function activate($id, $code=false)
	{
		...
		if ($activation)
		{
			...
			// Activation was done via email by the user - direct them to the success message
				...
				$this->_render('auth/activate-success');
			}
		}
		else
		{
	...
	function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->_render('auth/deactivate_user');
...
//				$username = strtolower($this->input->post('first_name')) . ' ' . strtolower($this->input->post('last_name'));
			...
			if ( $this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data)) { //check to see if we are creating the user
				...
				if ( ! $this->config->item('manual_activation', 'ion_auth') ) {
					$this->_render('auth/signup-success');
				}
				else {
					$this->_render('auth/signup-success-manual');
				}
			}
			else {
...
//		$this->load->view('auth/edit_user', $this->data);
	...
	function user_profile($id)
	{
		...
		$this->_render('auth/user_profile');
...
//		$this->data['current_groups'] = $this->ion_auth->get_users_groups($id)->result();
		...
		$this->_render('auth/user_edit_profile');
//		$this->load->view('auth/edit_user', $this->data);
	}
...
////		print_r($source_groups);
//		$this->data['source_groups'] = $source_groups;
//		$this->_render('auth/source_groups');
//	}
...
//			error_log("desc -> " .  $this->input->post('desc'));
			...
			if($new_group_id) {
				...
				$this->data['desc'] = "";
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				$this->_render('auth/create_group');
			}
		}
		else
		{
...
//		$this->load->view('auth/edit_group', $this->data);
	...
	function delete_group($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->_render('auth/delete_group');
		}
		else
		{
