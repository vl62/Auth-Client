...
//				error_log(print_r($validate_results, true));
    ...
    function do_upload_new($override = false) {
...
//			error_log($data['upload_data']['file_name'] . " -> " . $data['upload_data']['full_path'] . " -> " . $data['upload_data']['file_ext']);
            ...
            if (($validate_results['result_flag'])) {
                ...
                echo json_encode(array("status" => "duplicate", "error" => $data['upload_data']['file_name'] . ' IMPORT FAILED: ' . $validate_results['error']));
            }
        }
    }
...
//									error_log("STARTING $column_count -> $column");
                        ...
                        if (!empty($insert_data)) {
                            ...
                            $insert_id = $this->sources_model->checkVariants($insert_data);
                            if (!$insert_id) {
                                $return_data['result_flag'] = 0;
                                $return_data['error'] = "Duplicate records exist.";
                                return $return_data;
                            }
...
//									error_log("s");
            ...
            return $this->_validateFile($file, $extension, $source, $source_id, $sharing_policy, $fileformat, $mutalyzer_check);;
