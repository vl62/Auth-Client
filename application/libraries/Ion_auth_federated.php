...
class Ion_auth_federated extends Ion_auth
{
	/**
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $group_name = array(), $type = NULL) //need to test email activation
	{
		error_log("admin -> " . $type);
		...
		else
		{
			...
			{
				// Owen - added in type parameter to the function it's used here to allow a different email to be sent when an account is created through the admin interface
				if ( $type === "admin" ) {
					$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_activate_admin', 'ion_auth'), $data, true);
			...
			$this->session->set_flashdata('activation_email_unsuccessful', 'Unable to send activation email to user, please manually activate the account');
//			return FALSE; // Owen - edited out and added in setting flashdata session for when account is created by admin - reports that email was unable to be sent
			return $id;
		}
	}
